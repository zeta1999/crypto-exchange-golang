...
| 7 | Scenario & fault injection (test bed) | ◐ | price shift (arb dislocation) + latency + **scenario scripting** (JSONL timeline → runtime `Controls`) + **full trace replay** (`venue: replay` runs the whole emulator offline/deterministic from a recorded trace) + **cross-venue arb harness** (`ArbHarness` mirrors one reference into two shifted venues; `CrossArb` detector; test proves exploitable-then-closeable arb) + **deterministic clock** (`OrderBook.SetClock` + golden test, byte-reproducible runs) done, wired, reviewed/tested. Remaining: ack/fill latency at API edges, replay `speed` pacing. |
...
| 9 | Coinbase-compatible API | ☑ | `internal/api/coinbase` Advanced Trade REST (signed orders/batch_cancel/historical/accounts + time/product_book/products) **and WS** (level2/market_trades/user channels, message-based subscribe). CB-ACCESS HMAC **+ ES256 JWT** auth (ECDSA P-256, Bearer REST + WS), registry w/ hook fills (:8083). CI+race green; live-verified (incl. real openssl-signed JWT → 200, tampered → 401); reviewed. Deferred: true level2 diffs, fee/precision fields. |
