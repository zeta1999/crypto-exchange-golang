...
| 10 | Custody examples (stretch) | ◐ | **Phase 1 done:** `internal/custody` + `cmd/custody` testnet wallet/faucet toolkit. Encrypted keystore (AES-256-GCM + PBKDF2, secrets encrypted at rest, passphrase via `CUSTODY_PASSPHRASE`); pluggable `Chain`/`Faucet` + testnet-only `MustTestnet` guard; **XLM** (ed25519/StrKey, friendbot, Horizon) + **SOL** (ed25519/base58, devnet airdrop, getBalance). Encoders validated against real Circle USDC issuer/mint vectors; **live: friendbot funded a generated address w/ 10000 XLM**. CI+race green; reviewed (KDF-param validation, lamports bounds, no fake-success). Remaining: USDC-xlm/sol (trustline/ATA + Circle drip), ETH/USDT/ERC20 (Sepolia, secp256k1+keccak), Bitcoin testnet. |
