...
_Last updated: 2026-06-03 (CR-5)_
...
gRPC/WS request metrics); **Phase 10 custody** (stretch, testnet). **Coinbase CCXT ES256-JWT
conformance: PASS.** Recent edge polish: Binance `@depth` incremental diff stream, Coinbase fee
fields + **true level2 incremental diffs**, Solana USDC SPL send, and USDC/native deposit-watch
across Solana/EVM/BTC (auto-credit). Deferred: live broadcast of EVM/SOL/BTC sends (faucet-gated).
...
| 8 | Binance-compatible API | ☑ | `internal/api/binance` REST (signed order/cancel/openOrders/account + ping/time/**exchangeInfo**/depth/ticker) **and WS** (market @trade/@depth20 + **@depth incremental diffs** [depthUpdate U/u, REST lastUpdateId sync] + user-data executionReport via listenKey). HMAC auth, symbol map, registry w/ hook fills. **`exchangeInfo` unblocks CCXT loadMarkets** (symbols + PRICE_FILTER/LOT_SIZE/NOTIONAL). **`/account` reports live balances from the account ledger** (config-seeded; LIMIT orders lock funds → -2010 if short, settle on fill). **CR-5 OMS parity:** place-time idempotency on `newClientOrderId` (registry `RecordUnique` claims the id atomically → duplicate place rejected -2010 "Duplicate order sent.", never a 2nd resting order); **GET /api/v3/order** (signed, by orderId or origClientOrderId → full state incl. status/executedQty/cummulativeQuoteQty); `configs/oms-test.yaml` boot-verbatim preset (plain-HTTP edge on :8192, seeded balances, BTCUSDT↔BTC-USD, `emulator.enabled:false`). CI+race green; live-verified; reviewed. |
| 9 | Coinbase-compatible API | ☑ | `internal/api/coinbase` Advanced Trade REST (signed orders/batch_cancel/historical/accounts + time/product_book/**products list**+single) **and WS** (level2/market_trades/user channels, message-based subscribe). CB-ACCESS HMAC **+ ES256 JWT** auth (ECDSA P-256, Bearer REST + WS), registry w/ hook fills (:8083). **`products` list unblocks CCXT loadMarkets** (base/quote ids + increments + min size + trading_disabled). **`/accounts` reports live balances from the account ledger** (lock/settle, shared `internal/account`). **Stock `ccxt.NewCoinbase` (ES256 JWT) conformance PASS** — full signed lifecycle via endpoint swap; needed `brokerage/market/*` public-path aliases (ccxt >= v4.5); 401 on unsigned/tampered. CI+race green; live-verified; reviewed. **Fee fields**: orderView + WS userOrder total_fees/total_value_after_fees (configurable fee_rate), product price_increment. **level2 channel now emits true incremental diffs** (snapshot + changed-levels updates, removals as new_quantity "0", snapshot-first ordering). |
| 10 | Custody examples (stretch) | ☑ | **Testnet wallet/faucet toolkit complete** (`internal/custody` + `cmd/custody`, off by default, NOT wired into the server). **Phase 1:** `internal/custody` + `cmd/custody` testnet wallet/faucet toolkit. Encrypted keystore (AES-256-GCM + PBKDF2, secrets encrypted at rest, passphrase via `CUSTODY_PASSPHRASE`); pluggable `Chain`/`Faucet` + testnet-only `MustTestnet` guard; **XLM** (ed25519/StrKey, friendbot, Horizon) + **SOL** (ed25519/base58, devnet airdrop, getBalance) + **ETH/ERC20** (Sepolia; secp256k1+keccak EIP-55 addresses, eth_getBalance + ERC20 balanceOf incl. Circle USDC; manual faucet). Encoders validated against real Circle USDC issuer/mint + privkey=1 + EIP-55 spec vectors; **live: friendbot funded a generated address w/ 10000 XLM; Sepolia returns ETH+USDC balances**. CI+race green; reviewed (KDF-param validation, lamports bounds, no fake-success, no RPC-blip balance masking). **Phase 3 done:** USDC on XLM + SOL — `prepare` establishes a Stellar trustline (live: signed+submitted a real changeTrust tx; balance then shows USDC) using the stellar/go SDK; SOL SPL-token balance reading; Circle drip faucet (`CIRCLE_API_KEY`, falls back to the web faucet, blockchain ids env-overridable). Deps: decred secp256k1 + x/crypto/sha3 + stellar/go. CI+race green; reviewed (secret handling clean, testnet passphrase/endpoint consistent, parse-error propagation, secret zeroing). **Phase 4:** Bitcoin testnet — secp256k1 + hand-rolled bech32 native-segwit (P2WPKH `tb1…`) addresses, Esplora balances, manual faucet. bech32 validated against the BIP-173 vector; **live: a generated tb1 address is accepted by real Esplora**. CI+race green. **On-chain transfer flow** (`internal/transfer.Hub` + custody `Sender`): an arb bot withdraws from one venue (debit ledger → real testnet payment from its custody hot wallet) and the deposit watcher credits the destination venue's ledger on arrival. Exposed via Binance `/sapi/v1/capital/withdraw/apply` + native `POST /transfer`. **Live-verified: 100 XLM moved binance→coinbase on Stellar testnet** (ledger debit → real tx → watcher credit). Stellar live-verified full-loop; **EVM ERC20, Solana (SOL + USDC SPL TransferChecked), Bitcoin sends implemented + signing-vector/RPC-fake tested** (live broadcast faucet-gated). **Solana USDC loop closed in code**: the deposit watcher (`Received`) now detects USDC token-balance deltas (else SOL) so the hub auto-credits the destination ledger. Reviewed (precision-quantize leak fixed, honest withdraw errors, race-free watcher). |
| 11 | Hardening & observability | ☑ | `internal/metrics` (dependency-free Prometheus-text registry incl. a **Histogram** type, instrumented pipeline + API edges + **gRPC/native-WS request metrics & latency histograms**, `:9090/metrics`), `internal/ratelimit` (token bucket + keyed, capped; wired on both REST edges → 429/-1003), `config.Validate()` fail-fast, **scenario golden-file CI test** (deterministic emulator output pinned to a committed file). CI+race green; live-verified. Reviewed (histogram cumulativeness, WS label cardinality bounded). |
...
## Decisions log
- 2026-06-03 (CR-5): **Duplicate `newClientOrderId` → reject, not idempotent-return.**
  Real Binance answers a place that reuses a live clientOrderId with -2010 "Duplicate
  order sent."; the existing edge already maps insufficient-balance to -2010, so a
  reject is both Binance-faithful and consistent with the local error surface. The
  registry's `RecordUnique` performs the check-and-insert atomically under its lock, so
  concurrent duplicate places can never both create an order (race-tested). An empty
  newClientOrderId stays auto-generated and always unique.
- 2026-06-03 (CR-5): **`configs/oms-test.yaml`** is the documented OMS-test preset a
  consumer boots verbatim: Binance edge plain HTTP on fixed :8192, seeded
  USD/BTC balances, BTCUSDT↔BTC-USD, `emulator.enabled:false` (plain deterministic
  matching engine, no live feed). Rate limiting disabled (no 429s in tests).
