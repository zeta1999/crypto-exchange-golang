...
| 7 | Scenario & fault injection (test bed) | ◐ | price shift (arb dislocation) + latency + **scenario scripting** (JSONL timeline → runtime `Controls`) + **full trace replay** (`venue: replay` runs the whole emulator offline/deterministic from a recorded trace) done, wired, reviewed/tested. Remaining: cross-venue arb harness, ack/fill latency at API edges, replay `speed` pacing. |
