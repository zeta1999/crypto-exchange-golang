...
| 7 | Scenario & fault injection (test bed) | ◐ | price shift (arb dislocation) + latency injectors + **scenario scripting** (`scenario.go` JSONL timeline → runtime-mutable `Controls`, deterministic) done, wired, reviewed. Remaining: full trace replay through whole emulator, cross-venue arb harness, ack/fill latency at API edges. |
