...
| 8 | Binance-compatible API | ☑ | `internal/api/binance` REST (signed order/cancel/openOrders/account + ping/time/**exchangeInfo**/depth/ticker) **and WS** (market @trade/@depth20 + **@depth incremental diffs** [depthUpdate U/u, REST lastUpdateId sync] + user-data executionReport via listenKey). HMAC auth, symbol map, registry w/ hook fills. **`exchangeInfo` unblocks CCXT loadMarkets** (symbols + PRICE_FILTER/LOT_SIZE/NOTIONAL). **`/account` reports live balances from the account ledger** (config-seeded; LIMIT orders lock funds → -2010 if short, settle on fill). CI+race green; live-verified; reviewed. |
